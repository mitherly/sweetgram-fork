 ...
 * того, как включать.
 */
public class SweetgramPluginSettingsActivity extends UniversalFragment {

    private final SweetgramPlugins.Plugin plugin;
    ...
    public SweetgramPluginSettingsActivity(SweetgramPlugins.Plugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void read() {
        rows.clear();
        final String json = SweetgramHooks.declared(plugin.id);
        if (json == null) {
            return;
        }
        try {
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        read();
        if (rows.isEmpty()) {
            items.add(UItem.asShadow(LocaleController.getString(R.string.SweetgramPluginNoSettings)));
            return;
        }
    ...
    private String value(JSONObject row) {
        ...
        final String stored = SweetgramPluginHost.get(plugin.id, key, null);
        return stored != null ? stored : row.optString("default", "");
    }

    private void store(JSONObject row, String value) {
        final String key = row.optString("key");
        SweetgramPluginHost.set(plugin.id, key, value);
        SweetgramHooks.settingsChanged(plugin.id, key, value);
    ...
    protected void onClick(UItem item, View view, int position, float x, float y) {
        ...
        switch (row.optString("kind")) {
            ...
            case "action":
                ...
                SweetgramHooks.settingsChanged(plugin.id, row.optString("key"), null);
                break;
            default:
                break;
        }

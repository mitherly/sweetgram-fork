 ...
 */
@Keep
public class SweetgramUpdateLayout extends IUpdateLayout {
    ...
    @Keep
    public SweetgramUpdateLayout(Activity activity, ViewGroup sideMenuContainer) {
    ...
    private void onClick() {
        if (SweetgramUpdate.downloaded() != null) {
            SweetgramUpdate.install(activity);
        } else if (SweetgramUpdate.downloading()) {
            SweetgramUpdate.cancel();
        } else {
            SweetgramUpdate.download(() -> updateAppUpdateViews(account, true));
        }
    ...
    public void updateAppUpdateViews(int currentAccount, boolean animated) {
        ...
        account = currentAccount;
        if (SweetgramUpdate.available() != null) {
            ...
            if (SweetgramUpdate.downloaded() != null) {
                ...
                updateLayoutIcon.setProgress(SweetgramUpdate.progress(), true);
                updateTextView.setText(LocaleController.formatString(R.string.AppUpdateDownloading,
                        (int) (SweetgramUpdate.progress() * 100)), animated);
            } else {

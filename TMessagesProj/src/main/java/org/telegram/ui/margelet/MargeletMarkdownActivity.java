 ...
 */
public class MargeletMarkdownActivity extends UniversalFragment {
    ...
    private static final int[] TITLES = {
            ...
            R.string.Spoiler, R.string.Mono, R.string.MargeletMarkdownQuote,
            R.string.MargeletMarkdownQuoteCollapsed
    };
    ...
    protected CharSequence getTitle() {
        return LocaleController.getString(R.string.MargeletMarkdown);
    }
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        for (int i = 0; i < KINDS.length; i++) {
            items.add(UItem.asCheck(i, LocaleController.getString(TITLES[i]) + "   " + SAMPLES[i])
                    .setChecked(MargeletConfig.markdownEnabled(KINDS[i])));
        }
        items.add(UItem.asShadow(LocaleController.getString(R.string.MargeletMarkdownAbout)));
    }

    @Override
    protected void onClick(UItem item, View view, int position, float x, float y) {
        if (item.id >= 0 && item.id < KINDS.length) {
            MargeletConfig.setMarkdownEnabled(KINDS[item.id],
                    !MargeletConfig.markdownEnabled(KINDS[item.id]));

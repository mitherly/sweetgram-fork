 ...
 */
public class MargeletPluginHost {
    ...
    private static void notifyListeners() {
        ...
        synchronized (MargeletPluginHost.class) {
    ...
    public static synchronized void start() {
        if (started || !MargeletPlugins.anyEnabled()) {
            return;
        }
        started = true;
        worker = new Thread(() -> {
            ...
            try {
                ...
                FileLog.e(t);
                log("margelet", "питон не поднялся: " + t, true);
                return;
            }
            log("margelet", "питон готов", false);
            for (MargeletPlugins.Plugin plugin : MargeletPlugins.installed()) {
                if (plugin.enabled() && MargeletConfig.pluginsEnabled()) {
                    run(plugin);
                }
            }
            Looper.loop();
     ...
     */
    public static void launch(MargeletPlugins.Plugin plugin) {
        start();
    ...
    /**
     * Зовёт MargeletPython из модуля приложения.
     *

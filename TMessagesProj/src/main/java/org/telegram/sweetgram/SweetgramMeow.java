...
public class SweetgramMeow {
    ...
    public static void play(Context context) {
        if (!SweetgramConfig.meowEnabled()) {
            return;
        }
        try {
            MediaPlayer player;
            final String own = SweetgramConfig.meowPath();
            if (own != null && new File(own).exists()) {
                player = new MediaPlayer();
                player.setDataSource(own);
                player.prepare();
            } else {
                player = MediaPlayer.create(context, R.raw.sweetgram_meow);
            }
            if (player == null) {
                return;
            }

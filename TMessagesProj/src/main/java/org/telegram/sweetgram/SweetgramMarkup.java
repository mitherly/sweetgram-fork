...
public class SweetgramMarkup {
    ...
    public static CharSequence encode(CharSequence text) {
        ...
        final SweetgramSpans.Base[] all = spanned.getSpans(0, spanned.length(), SweetgramSpans.Base.class);
        final ArrayList<SweetgramSpans.Base> fresh = new ArrayList<>();
        for (SweetgramSpans.Base span : all) {
            if (!span.decoded) {
                fresh.add(span);
            }
        }
        final SweetgramSpans.Base[] spans = fresh.toArray(new SweetgramSpans.Base[0]);
        ...
        for (SweetgramSpans.Base span : spans) {
        ...
        if (!SweetgramConfig.watermarkOnSend()) {
            return out;
        }
        return out.append("\n").append(HEADER);
    }
    ...
    public static void injectEntities(CharSequence text, java.util.ArrayList<org.telegram.tgnet.TLRPC.MessageEntity> entities) {
        ...
        for (Run run : parse(text)) {
            if (run.kind != KIND_BUTTON || !SweetgramConfig.markupEnabled(KIND_BUTTON)) {
                continue;
            }
    ...
    public static void apply(Spannable text) {
        ...
        for (Run run : parse(text)) {
            if (!SweetgramConfig.markupEnabled(run.kind)) {
                continue;   // этот вид оформления человек выключил у себя
            }
            if (run.kind == KIND_BUTTON) {
                final String url = run.text();
                text.setSpan(new SweetgramSpans.Button(run.value, url),
            ...
            if (run.kind == KIND_OUTLINE) {
                ...
                while (i < run.end) {
                    ...
                    if (i > word) {
                        text.setSpan(new SweetgramSpans.Outline(run.value), word, i,
            ...
            final Object span = SweetgramSpans.create(run.kind, run.value);
            if (span != null) {
                if (span instanceof SweetgramSpans.Base) {
                    // Метки этого куска уже в тексте: при отправке их не
                    // ставить заново.
                    ((SweetgramSpans.Base) span).decoded = true;
                }
                text.setSpan(span, run.start, run.end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        if (!SweetgramConfig.showWatermarks()) {
            hideHeader(text);
        }
    }
    ...
    private static void hideHeader(Spannable text) {
        ...
        text.setSpan(new SweetgramSpans.Hidden(), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

...
Плагину доступен модуль sweetgram — через него он и живёт: пишет в консоль,
...
class _Task(dynamic_proxy(Runnable)):
    ...
    про очереди и обёртки, ему достаточно sweetgram.every.
    """
...
class Sweetgram:
    ...
    def every(self, ms, call):
        """Повторять каждые ms миллисекунд. Остановить — sweetgram.cancel(...)."""
    ...
    def cancel(self, task=None):
        ...
        написал sweetgram.cancel() в обработчике отправки, имея в виду «не
    ...
    def send(self, chat, text):
        ...
        в sweetgram.background, а когда ответ пришёл — отправил его отсюда.
        """
    ...
    def dont_send(self):
        ...
        Заведено потому, что за этим тянулись рукой к sweetgram.cancel — а
    ...
    def on_send(self, call):
        ...
        отсюда надо уносить в sweetgram.ui или sweetgram.every.
        """
    ...
    def switch(self, key, title, default=False, about=None):
        """Переключатель. Читается через sweetgram.flag(key)."""
    ...
    def text(self, key, title, default="", about=None):
        """Строка, которую человек вписывает сам. Читается через sweetgram.get(key)."""
    ...
    def choice(self, key, title, options, default=None):
        """Выбор одного из нескольких. Читается через sweetgram.get(key)."""
...
def sending(text, dialog_id):
    ...
    for sweetgram in list(_plugins.values()):
        for call in list(sweetgram._on_send):
            ...
            # сказали: sweetgram.cancel() или sweetgram.dont_send() внутри
...
def settings_changed(plugin_id, key, value):
    ...
    if action is not None:
        try:
            ...
            _Host.log(sweetgram.name, traceback.format_exc(), True)
        return
    for call in list(sweetgram._on_settings):
        try:
            call(key, value)
        except Exception:
            _Host.log(sweetgram.name, traceback.format_exc(), True)
